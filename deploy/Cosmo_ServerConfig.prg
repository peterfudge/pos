...
IF FILE("WCONNECT.APP")
   MessageBox("This feature only works in the full version of Web Connection",48,"Cosmo")
   RETURN
ENDIF
...
IF !IsAdmin() 
   MESSAGEBOX("Admin privileges are required to configure the Web Server." + CHR(13) +;
              ...
              48,"Cosmo Server Configuration")
   RETURN
ENDIF

*** Try to read from Cosmo.ini [ServerConfig] section
loApi = CREATEOBJECT("wwAPI")
lcIniPath = FULLPATH("Cosmo.ini")
...
IF ISNULLOREMPTY(lcScriptMaps)
  lcScriptMaps = "wc,wcs,wcsx,cosmo"  
ENDIF
IF EMPTY(lcIISPath)
...
IF FILE("Cosmo.exe")
   RUN /n4 "Cosmo.exe" /regserver

   *** Optionally set DCOM permission - only set if needed
   *** requires that DComLaunchPermissions.exe is available
   * DCOMLaunchPermissions("Cosmo.CosmoServer","INTERACTIVE")
   * DCOMLaunchPermissions("Cosmo.CosmoServer","username","password")
ENDIF
